...
const AdminDashboard = () => {
  ...
  const fetchSoftware = async () => {
    try {
      const response = await axios.get('http://localhost:3000/api/software');
  ...
  const handleCreateSoftware = async (e) => {
    ...
    setSuccess('');
    try {
      await axios.post('http://localhost:3000/api/software', { name, description, accessLevels });
  ...
  const handleManagerChange = async (e) => {
    ...
    setManagerErr('');
    try {
      const res = await axios.patch('http://localhost:3000/api/users/manager-email', {
        email: managerEmail,
        name: managerName,
